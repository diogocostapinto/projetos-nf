...
O que cada aprovado enxerga é controlado pelo **escopo por operador**: por padrão o operador vê todos os projetos; na tela Equipe o admin pode restringir (botão *Vê tudo / Restrito*) e marcar só os projetos liberados para aquela pessoa.

Além disso, o admin pode marcar um **projeto** ou um **setor** como **privado** (🔒, no modal Editar projeto): itens privados só aparecem para administradores — operadores não veem o item, suas tarefas, nem o botão de tornar privado. Aplicado no servidor (RLS: `can_view_project`/`can_see_task`).

Em qualquer projeto que o operador **vê**, ele tem controle total das tarefas: **criar, editar qualquer campo** (título, descrição, prazo, prioridade, setor, responsável, situação) e **excluir**. Operadores também podem **adicionar e renomear setores** em qualquer projeto (Projetos ou Desenvolvimento) e **criar projetos novos na aba Desenvolvimento** (viram donos e passam a vê-los mesmo se restritos). O que operadores **não** fazem: criar projetos "normais", editar dados do projeto (nome/situação/cor), reordenar ou excluir projetos, nem excluir setores — isso é exclusivo do admin. O **admin** vê tudo, aprova/revoga acessos, define escopos e gerencia tudo. As regras são aplicadas no servidor (RLS + triggers que restringem, por coluna, o que cada papel altera), não apenas escondidas na interface. Cada tarefa registra a **data de criação**, exibida no card e ao editar, para acompanhar há quanto tempo está aberta. O sino no cabeçalho (🔔) mostra as **novas tarefas atribuídas** ao usuário logado, com confirmação de recebimento por item ou um botão "marcar todas como lidas" (o estado fica no banco, em `task_acks`, e vale entre dispositivos).
